...
class CircleConstraintForCar:
	...
	def evaluate_constraint_J(self, x, u):
		#evolve the system for one to evaluate constraint
		x_next = self.system.transition(x, np.zeros(self.system.control_size))
		result = np.zeros(x.shape)
		result[0] = -2*(x_next[0] - self.center[0])
		result[1] = -2*(x_next[1] - self.center[1])
		result[2] = 2*(x_next[0] - self.center[0]) * self.system.dt * np.sin(x[2]) - 2*(x_next[1] - self.center[1]) * self.system.dt * np.cos(x[2])
		#result[3] = -2*(x_next[1] - self.center[1]) * self.system.dt * np.sin(x[2]) -2*(x_next[0] - self.center[0]) * self.system.dt * np.cos(x[2])
		#result[3] =0
